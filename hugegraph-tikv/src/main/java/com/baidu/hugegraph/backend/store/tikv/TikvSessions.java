...
public class TikvSessions extends BackendSessionPool {

    private final String namespace;
    private Connection hbase;

    public TikvSessions(HugeConfig config, String namespace, String store) {
        super(config, namespace + "/" + store);
        this.namespace = namespace;
    }

    protected Connection tikv() {
    ...
    private AggregationClient aggregationClient() {
        ...
        long timeout = this.config().get(TikvOptions.AGGR_TIMEOUT);
    ...
    public synchronized void open() throws IOException {
        ...
        String hosts = config.get(TikvOptions.HBASE_HOSTS);
        int port = config.get(TikvOptions.HBASE_PORT);
        String znodeParent = config.get(TikvOptions.HBASE_ZNODE_PARENT);
        boolean isEnableKerberos = config.get(TikvOptions.HBASE_KERBEROS_ENABLE);
        ...
        hConfig.setInt("zookeeper.recovery.retry",
                       config.get(TikvOptions.HBASE_ZK_RETRY));

        // Set hbase.hconnection.threads.max 64 to avoid OOM(default value: 256)
        hConfig.setInt("hbase.hconnection.threads.max",
                       config.get(TikvOptions.HBASE_THREADS_MAX));

        String hbaseSite = config.get(TikvOptions.HBASE_HBASE_SITE);
        hConfig.addResource(new Path(hbaseSite));

        if(isEnableKerberos) {
            String krb5Conf = config.get(TikvOptions.HBASE_KRB5_CONF);
            System.setProperty("java.security.krb5.conf", krb5Conf);
            String principal = config.get(TikvOptions.HBASE_KERBEROS_PRINCIPAL);
            String keyTab = config.get(TikvOptions.HBASE_KERBEROS_KEYTAB);
     ...
     */
    public interface TikvSession<R> {
    ...
    public class Session extends AbstractBackendSession
                         implements TikvSession<RowIterator> {
        ...
        public Connection hbase() {
            return TikvSessions.this.hbase();
        }

        public String namespace() {
            return TikvSessions.this.namespace;
        }
        ...
        public boolean closed() {
            return !this.opened || !TikvSessions.this.opened();
        }
         ...
         */
        public long storeSize(String table) throws IOException {
            return TikvSessions.this.storeSize(table);
        }
    ...
    public class CountSession implements TikvSession<Number>, AutoCloseable {
