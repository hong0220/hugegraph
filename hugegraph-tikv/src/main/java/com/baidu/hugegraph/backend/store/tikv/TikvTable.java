...
import com.baidu.hugegraph.backend.store.tikv.TikvSessions.RowIterator;
import com.baidu.hugegraph.backend.store.tikv.TikvSessions.CountSession;
import com.baidu.hugegraph.backend.store.tikv.TikvSessions.Session;
import com.baidu.hugegraph.backend.store.tikv.TikvSessions.TikvSession;
...
public class TikvTable extends BackendTable<Session, BackendEntry> {
    ...
    public TikvTable(String table) {
        super(table);
        this.shardSpliter = new TikvShardSpliter(table);
    }
    ...
    public Iterator<BackendEntry> query(Session session, Query query) {
        ...
        TikvSession<RowIterator> tikvSession = session;
        return this.newEntryIterator(query, this.query(tikvSession, query));
    }

    protected <R> R query(TikvSession<R> session, Query query) {
        // Query all
        if (query.empty()) {
    ...
    protected <R> R queryAll(TikvSession<R> session, Query query) {
        if (query.paging()) {
    ...
    protected <R> R queryById(TikvSession<R> session, Id id) {
        return session.get(this.table(), null, id.asBytes());
    }

    protected <R> R queryByIds(TikvSession<R> session, Set<Id> ids) {
    ...
    protected <R> R queryByPrefix(TikvSession<R> session,
                                  IdPrefixQuery query) {
        return session.scan(this.table(), query.start().asBytes(),
                            query.inclusiveStart(), query.prefix().asBytes());
    }

    protected <R> R queryByRange(TikvSession<R> session, IdRangeQuery query) {
    ...
    protected <R> R queryByCond(TikvSession<R> session, ConditionQuery query) {
    ...
    protected <R> R queryByRange(TikvSession<R> session,
