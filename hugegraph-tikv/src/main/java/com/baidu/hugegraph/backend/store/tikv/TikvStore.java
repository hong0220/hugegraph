...
public abstract class TikvStore extends AbstractBackendStore<Session> {
    ...
    public synchronized void open(HugeConfig config) {
        ...
        if (this.sessions == null) {
            this.sessions = new TikvSessions(config, this.namespace, this.store);
        }
    ...
    public void init() {
        ...
        for (String table : this.tableNames()) {
            try {
                this.sessions.createTable(table, TikvTable.cfs());
            } catch (IOException e) {
                throw new BackendException(
    ...
    public void truncate() {
        ...
        long timeout = this.sessions.config().get(TikvOptions.TRUNCATE_TIMEOUT);
    ...
    private final void checkConnectionOpened() {
        E.checkState(this.sessions != null && this.sessions.opened(),
                     "Tikv store has not been initialized");
    }
    ...
    public static class TikvSchemaStore extends TikvStore {
        ...
        public TikvSchemaStore(BackendStoreProvider provider,
            ...
            this.counters = new TikvTables.Counters();

            registerTableManager(HugeType.VERTEX_LABEL,
                                 new TikvTables.VertexLabel());
            registerTableManager(HugeType.EDGE_LABEL,
                                 new TikvTables.EdgeLabel());
            registerTableManager(HugeType.PROPERTY_KEY,
                                 new TikvTables.PropertyKey());
            registerTableManager(HugeType.INDEX_LABEL,
                                 new TikvTables.IndexLabel());
    ...
    public static class TikvGraphStore extends TikvStore {
        ...
        public TikvGraphStore(BackendStoreProvider provider,
            ...
            registerTableManager(HugeType.RANGE_INT_INDEX,
                                 TikvTables.RangeIndex.rangeInt(store));
            registerTableManager(HugeType.RANGE_FLOAT_INDEX,
                                 TikvTables.RangeIndex.rangeFloat(store));
            registerTableManager(HugeType.RANGE_LONG_INDEX,
                                 TikvTables.RangeIndex.rangeLong(store));
            registerTableManager(HugeType.RANGE_DOUBLE_INDEX,
                                 TikvTables.RangeIndex.rangeDouble(store));
